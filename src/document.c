...
#define UPSKIRT_LI_END 8	/* internal list flag */

const char *sd_find_block_tag(const char *str, unsigned int len);
...
typedef size_t
(*char_trigger)(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);

static size_t char_emphasis(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_quote(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_linebreak(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_codespan(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_escape(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_entity(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_langle_tag(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_autolink_url(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_autolink_email(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_autolink_www(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_link(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_image(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_superscript(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_math(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_ref(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size);

void sub_render(sd_document *doc, sd_buffer *ob, const uint8_t *data, size_t size, int position);
...
struct sd_document {
	...
	sd_stack work_bufs[2];
	sd_extensions ext_flags;
	size_t max_nesting;
	int in_link_body;
};
...
static sd_buffer *
newbuf(sd_document *doc, int type)
{
	static const size_t buf_size[2] = {256, 64};
	sd_buffer *work = NULL;
	sd_stack *pool = &doc->work_bufs[type];
	...
	if (pool->size < pool->asize &&
		...
		work = sd_buffer_new(buf_size[type]);
...
static void
popbuf(sd_document *doc, int type)
{
	doc->work_bufs[type].size--;
}

static void
unscape_text(sd_buffer *ob, sd_buffer *src)
{
	size_t i = 0, org;
	while (i < src->size) {
		org = i;
		...
		if (i > org)
			sd_buffer_put(ob, src->data + org, i - org);

		if (i + 1 >= src->size)
			break;

		sd_buffer_putc(ob, src->data[i + 1]);
		i += 2;
	}
}
...
{
	struct link_ref *ref = sd_calloc(1, sizeof(struct link_ref));
...
free_link_refs(struct link_ref **references)
{
	...
	for (i = 0; i < REF_TABLE_SIZE; ++i) {
		...
		while (r) {
			next = r->next;
			sd_buffer_free(r->link);
			sd_buffer_free(r->title);
			free(r);
			r = next;
		}
	}
...
{
	struct footnote_ref *ref = sd_calloc(1, sizeof(struct footnote_ref));
	ref->id = hash_link_ref(name, name_size);
...
{
	struct footnote_item *item = sd_calloc(1, sizeof(struct footnote_item));
	if (!item)
		return 0;
	item->ref = ref;
 ...
 */
static void
replace_spacing(sd_buffer *ob, const uint8_t *data, size_t size)
{
	size_t i = 0, mark;
	sd_buffer_grow(ob, size);
	while (1) {
		mark = i;
		while (i < size && data[i] != '\n') i++;
		sd_buffer_put(ob, data + mark, i - mark);
...
static size_t
tag_length(uint8_t *data, size_t size, sd_autolink_type *autolink)
{
	size_t i, j;
	...
	/* scheme test */
	*autolink = UPSKIRT_AUTOLINK_NONE;
	...
	if (i > 1 && data[i] == '@') {
		if ((j = is_mail_autolink(data + i, size - i)) != 0) {
			*autolink = UPSKIRT_AUTOLINK_EMAIL;
			return i + j;
		}
	}

	if (i > 2 && data[i] == ':') {
		*autolink = UPSKIRT_AUTOLINK_NORMAL;
		i++;
	}
...
static void
parse_inline(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t size)
{
	size_t i = 0, end = 0, consumed = 0;
	sd_buffer work = { 0, 0, 0, 0, NULL, NULL, NULL };
...
static size_t
parse_emph1(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t size, uint8_t c)
{
	size_t i = 0, len;
	sd_buffer *work = 0;
	int r;
	...
	while (i < size) {
		...
		if (data[i] == c && !_isspace(data[i - 1])) {
			...
			if (doc->ext_flags & UPSKIRT_EXT_NO_INTRA_EMPHASIS) {
			...
			if (doc->ext_flags & UPSKIRT_EXT_UNDERLINE && c == '_')
...
static size_t
parse_emph2(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t size, uint8_t c)
{
	size_t i = 0, len;
	sd_buffer *work = 0;
	int r;
...
static size_t
parse_emph3(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t size, uint8_t c)
{
	size_t i = 0, len;
	int r;
	...
	while (i < size) {
		...
		if (i + 2 < size && data[i + 1] == c && data[i + 2] == c && doc->md.triple_emphasis) {
			/* triple symbol found */
			sd_buffer *work = newbuf(doc, BUFFER_SPAN);
...
static size_t
parse_math(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size, const char *end, size_t delimsz, int displaymode)
{
	sd_buffer text = { NULL, 0, 0, 0, NULL, NULL, NULL };
	size_t i = delimsz;
	...
	i += delimsz;
	if (delimsz == 2 && !(doc->ext_flags & UPSKIRT_EXT_MATH_EXPLICIT))
...
static size_t
parse_include(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size)
{
	/* @include(path) */
	size_t i = 9;
	size_t n = 0;
...
static size_t
char_emphasis(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size)
{
	uint8_t c = data[0];
	size_t ret;
...
static size_t
char_linebreak(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size)
{
...
static size_t
char_codespan(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size)
{
	sd_buffer work = { NULL, 0, 0, 0, NULL, NULL, NULL };
...
static size_t
char_quote(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size)
{
...
static size_t
char_escape(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size)
{
	...
	if (size > 1) {
		if (data[1] == '\\' && (doc->ext_flags & UPSKIRT_EXT_MATH) &&
...
static size_t
char_entity(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size)
{
	size_t end = 1;
	sd_buffer work = { 0, 0, 0, 0, NULL, NULL, NULL };
	...
	else sd_buffer_put(ob, data, end);
...
static size_t
char_langle_tag(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size)
{
	sd_buffer work = { NULL, 0, 0, 0, NULL, NULL, NULL };
	sd_autolink_type altype = UPSKIRT_AUTOLINK_NONE;
	...
	if (end > 2) {
		if (doc->md.autolink && altype != UPSKIRT_AUTOLINK_NONE) {
			sd_buffer *u_link = newbuf(doc, BUFFER_SPAN);
			work.data = data + 1;
			work.size = end - 2;
...
static size_t
char_autolink_www(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size)
{
	sd_buffer *link, *link_url, *link_text;
	...
	if ((link_len = sd_autolink__www(&rewind, link, data, offset, size, UPSKIRT_AUTOLINK_SHORT_DOMAINS)) > 0) {
		link_url = newbuf(doc, BUFFER_SPAN);
		UPSKIRT_BUFPUTSL(link_url, "http://");
		sd_buffer_put(link_url, link->data, link->size);
...
static size_t
char_ref(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size)
{
...
static size_t
char_autolink_email(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size)
{
	...
	if ((link_len = sd_autolink__email(&rewind, link, data, offset, size, 0)) > 0) {
		...
		doc->md.autolink(ob, link, UPSKIRT_AUTOLINK_EMAIL, &doc->data);
	}
...
static size_t
char_autolink_url(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size)
{
	sd_buffer *link;
	size_t link_len, rewind;
	...
	if ((link_len = sd_autolink__url(&rewind, link, data, offset, size, 0)) > 0) {
		...
		doc->md.autolink(ob, link, UPSKIRT_AUTOLINK_NORMAL, &doc->data);
	}
...
static size_t
char_image(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size) {
	size_t ret;
...
static size_t
char_link(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size)
{
	int is_img = (offset && data[-1] == '!' && !is_escaped(data - offset, offset - 1));
	int is_footnote = (doc->ext_flags & UPSKIRT_EXT_FOOTNOTES && data[1] == '^');
	size_t i = 1, txt_e, link_b = 0, link_e = 0, title_b = 0, title_e = 0;
	sd_buffer *content = NULL;
	sd_buffer *link = NULL;
	sd_buffer *title = NULL;
	sd_buffer *u_link = NULL;
	...
	/* footnote link */
	if (is_footnote) {
		sd_buffer id = { NULL, 0, 0, 0, NULL, NULL, NULL };
	...
	if (i < size && data[i] == '(') {
		...
		if (link_e > link_b) {
			link = newbuf(doc, BUFFER_SPAN);
			sd_buffer_put(link, data + link_b, link_e - link_b);
		}

		if (title_e > title_b) {
			title = newbuf(doc, BUFFER_SPAN);
			sd_buffer_put(title, data + title_b, title_e - title_b);
		}
	...
	/* reference style link */
		...
		sd_buffer *id = newbuf(doc, BUFFER_SPAN);
		struct link_ref *lr;
		...
		if (link_b == link_e)
			...
			sd_buffer_put(id, data + link_b, link_e - link_b);
	...
	/* ruby text extension */
	else if (i < size && data[i] == '{' && doc->ext_flags & UPSKIRT_EXT_RUBY) {
		...
		if (txt_e > 1) {
			...
			sd_buffer_put(content, data + 1, txt_e - 1);
		}
	...
	/* citation extension */
			 ...
			 doc->ext_flags & UPSKIRT_EXT_CITE) {

		/* building content */
		content = newbuf(doc, BUFFER_SPAN);
		sd_buffer_put(content, data + 2, txt_e	- 3);
...
static size_t
char_superscript(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size)
{
...
static size_t
char_math(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t offset, size_t size)
{
	/* double dollar */
	if (size > 1 && data[1] == '$')
	...
	if (doc->ext_flags & UPSKIRT_EXT_MATH_EXPLICIT)
...
static size_t
parse_codefence(uint8_t *data, size_t size, sd_buffer *lang, size_t *width, uint8_t *chr)
{
...
static int
is_atxheader(sd_document *doc, uint8_t *data, size_t size)
{
	if (data[0] != '#')
		return 0;

	if (doc->ext_flags & UPSKIRT_EXT_SPACE_HEADERS) {
		size_t level = 0;
...
static void parse_block(sd_buffer *ob, sd_document *doc,
			uint8_t *data, size_t size, int position);


/* parse_blockquote • handles parsing of a blockquote fragment */
static size_t
parse_blockquote(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t size)
{
	...
	while (beg < size) {
		...
		if (beg < end) { /* copy into the in-place working buffer */
			/* sd_buffer_put(work, data + beg, end - beg); */
			if (!work_data)
				work_data = data + beg;
...
static size_t
parse_htmlblock(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t size, int do_render);

/* parse_blockquote • handles parsing of a regular paragraph */
static size_t
parse_paragraph(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t size)
{
	sd_buffer work = { NULL, 0, 0, 0, NULL, NULL, NULL };
	size_t i = 0, end = 0;
	int level = 0;
...
static size_t
parse_fencedcode(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t size)
{
	sd_buffer text = { 0, 0, 0, 0, NULL, NULL, NULL };
	sd_buffer lang = { 0, 0, 0, 0, NULL, NULL, NULL };
...
static size_t
parse_blockcode(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t size)
{
	size_t beg, end, pre;
	sd_buffer *work = 0;
	...
	while (beg < size) {
		...
		if (beg < end) {
			...
			if (is_empty(data + beg, end - beg))
				sd_buffer_putc(work, '\n');
			else sd_buffer_put(work, data + beg, end - beg);
		}
		beg = end;
	}
...
static size_t
parse_listitem(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t size, sd_list_flags *flags)
{
	sd_buffer *work = 0, *inter = 0;
	...
	if (doc->ext_flags & UPSKIRT_EXT_FENCED_CODE) {
	...
	while (beg < size) {
		...
		if ((has_next_uli && !is_hrule(data + beg + i, end - beg - i)) || has_next_oli) {
			...
			if (pre <= orgpre) {
				...
				if (in_empty && (
					((*flags & UPSKIRT_LIST_ORDERED) && has_next_uli) ||
					(!(*flags & UPSKIRT_LIST_ORDERED) && has_next_oli)))
					*flags |= UPSKIRT_LI_END;
		...
		sd_buffer_put(work, data + beg + i, end - beg - i);
		beg = end;
	}

	/* render of li contents */
	if (has_inside_empty)
		*flags |= UPSKIRT_LI_BLOCK;

	if (*flags & UPSKIRT_LI_BLOCK) {
...
static size_t
parse_list(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t size, sd_list_flags flags)
{
	sd_buffer *work = 0;
	size_t i = 0, j;
...
static size_t
parse_atxheader(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t size)
{
	size_t level = 0;
	size_t skip = 0;
...
static void
parse_footnote_def(sd_buffer *ob, sd_document *doc, unsigned int num, uint8_t *data, size_t size)
{
	sd_buffer *work = 0;
	work = newbuf(doc, BUFFER_SPAN);
...
static void
parse_footnote_list(sd_buffer *ob, sd_document *doc, struct footnote_list *footnotes)
{
	sd_buffer *work = 0;
...
parse_table_row(
	...
	size_t size,
	size_t columns,
	sd_table_flags *col_data,
	sd_table_flags header_flag)
{
	size_t i = 0, col, len;
	sd_buffer *row_work = 0;
	...
	for (; col < columns; ++col) {
		sd_buffer empty_cell = { 0, 0, 0, 0, NULL, NULL, NULL };
	...
	sd_table_flags **column_data)
{
	...
	*columns = pipes + 1;
	*column_data = sd_calloc(*columns, sizeof(sd_table_flags));
	...
	for (col = 0; col < *columns && i < under_end; ++col) {
		...
		if (data[i] == ':') {
			i++; (*column_data)[col] |= UPSKIRT_TABLE_ALIGN_LEFT;
			dashes++;
		}
		...
		if (i < under_end && data[i] == ':') {
			i++; (*column_data)[col] |= UPSKIRT_TABLE_ALIGN_RIGHT;
			dashes++;
		}
	...
	size_t size)
{
	...
	sd_buffer *work = 0;
	sd_buffer *header_work = 0;
	sd_buffer *body_work = 0;

	size_t columns;
	sd_table_flags *col_data = NULL;
	...
	size_t size)
{
	...
	if (doc->md.abstract)
	{
		...
		if (doc->md.keywords && doc->document_metadata->keywords)
		{
			...
			sd_buffer_puts(b, doc->document_metadata->keywords);
	...
	size_t size)
{
	...
	if (doc->md.opn_equation && skip)
	{
		...
		sd_buffer * text = sd_buffer_new(skip);
		sd_buffer_put(text, data+begin, skip);
		if (doc->md.eq_math)
...
static void
parse_position(sd_buffer *ob, sd_document *doc){
	if (doc->md.position){
		doc->md.position(ob);
	}
}
...
/* parse_block • parsing of one block, returning next uint8_t to parse */
static void
parse_block(sd_buffer *ob, sd_document *doc, uint8_t *data, size_t size, int position)
{
	size_t beg, end, i;
	uint8_t *txt_data;
	beg = 0;
	...
	while (beg < size) {
		...
		if (is_atxheader(doc, txt_data, end))
		...
		else if ((doc->ext_flags & UPSKIRT_EXT_FENCED_CODE) != 0 &&
			(i = parse_fencedcode(ob, doc, txt_data, end)) != 0)
			beg += i;

		else if ((doc->ext_flags & UPSKIRT_EXT_TABLES) != 0 &&
		...
		else if (!(doc->ext_flags & UPSKIRT_EXT_DISABLE_INDENTED_CODE) && prefix_code(txt_data, end))
			...
			beg += parse_list(ob, doc, txt_data, end, UPSKIRT_LIST_ORDERED);
...
is_footnote(const uint8_t *data, size_t beg, size_t end, size_t *last, char* base_folder, struct footnote_list *list)
{
	...
	while (i < end) {
		...
		if (ind == 0) {
			...
			sd_buffer_putc(contents, '\n');
		}

		in_empty = 0;

		/* adding the line into the content buffer */
		sd_buffer_put(contents, data + start + ind, i - start - ind);
		/* add carriage return */
		if (i < end) {
			sd_buffer_putc(contents, '\n');
			if (i < end && (data[i] == '\n' || data[i] == '\r')) {
...
/*	int n; */
	...
	if (refs) {
		...
		ref->link = sd_buffer_new(link_end - link_offset);
		sd_buffer_put(ref->link, data + link_offset, link_end - link_offset);

		if (title_end > title_offset) {
			ref->title = sd_buffer_new(title_end - title_offset);
			sd_buffer_put(ref->title, data + title_offset, title_end - title_offset);
		}
	}
...
static void expand_tabs(sd_buffer *ob, const uint8_t *line, size_t size)
{
	...
	while (i < size) {
		...
		if (i > org)
			sd_buffer_put(ob, line + org, i - org);

		if (i >= size)
			break;

		do {
			sd_buffer_putc(ob, ' '); tab++;
		} while (tab % 4);
...
sd_document_new(
	...
	size_t max_nesting)
{
	sd_document *doc = NULL;

	assert(max_nesting > 0 && renderer);

	doc = sd_malloc(sizeof(sd_document));
	memcpy(&doc->md, renderer, sizeof(sd_renderer));
	...
	sd_stack_init(&doc->work_bufs[BUFFER_BLOCK], 4);
	sd_stack_init(&doc->work_bufs[BUFFER_SPAN], 8);

	memset(doc->active_char, 0x0, 256);

	if (extensions & UPSKIRT_EXT_UNDERLINE && doc->md.underline) {
	...
	if (doc->md.emphasis || doc->md.double_emphasis || doc->md.triple_emphasis) {
		...
		doc->active_char['_'] = MD_CHAR_EMPHASIS;
		if (extensions & UPSKIRT_EXT_STRIKETHROUGH)
			doc->active_char['~'] = MD_CHAR_EMPHASIS;
		if (extensions & UPSKIRT_EXT_HIGHLIGHT)
			doc->active_char['='] = MD_CHAR_EMPHASIS;
	}
...
size_t
skip_yaml(sd_document *doc, sd_buffer *ob, const uint8_t *data, size_t size)
{
	size_t skip = 0;
...
void
sub_render(sd_document *doc, sd_buffer *ob, const uint8_t *data, size_t size, int position)
{
	static const uint8_t UTF8_BOM[] = {0xEF, 0xBB, 0xBF};

	sd_buffer *text;
	size_t beg, end;
	text = sd_buffer_new(64);

	/* Preallocate enough space for our buffer to avoid expanding while copying */
	sd_buffer_grow(text, size);
	/* first pass: looking for references, copying everything else */
	beg = 0;

	int footnotes_enabled = doc->ext_flags & UPSKIRT_EXT_FOOTNOTES;
	...
	sd_buffer_grow(ob, text->size + (text->size >> 1));
...
void
render_metadata(sd_document *doc, sd_buffer *ob, metadata * meta)
{

	if (meta->title != NULL && doc->md.title)
	{
		sd_buffer * b = sd_buffer_new(1);
		sd_buffer_puts(b, meta->title);
	...
	{
		sd_buffer * b = sd_buffer_new(1);
		sd_buffer_puts(b, meta->affiliation);
...
void
check_for_ref(sd_document *doc, const uint8_t *data, size_t size, html_counter * counter, float_type type)
{
	int caption = 0;
	size_t i = 0;
...
void
look_for_ref(sd_document *doc, const uint8_t *data, size_t size, html_counter * counter)
{
...
void
find_references(sd_document *doc, const uint8_t *data, size_t size, html_counter * counter)
{
	size_t i;
	for (i = 0; i < size; i++)
	{
...
toc *
generate_toc(sd_document * doc, const uint8_t * data, size_t size, toc* parent)
{
	if (!data || !size)
		return parent;
	size_t i = 0;
...
void
sd_document_render(sd_document *doc, sd_buffer *ob, const uint8_t *data, size_t size, int position)
{
	...
	footnotes_enabled = doc->ext_flags & UPSKIRT_EXT_FOOTNOTES;
...
void
sd_document_render_inline(sd_document *doc, sd_buffer *ob, const uint8_t *data, size_t size, int position)
{
	size_t i = 0, mark;
	sd_buffer *text = sd_buffer_new(64);
...
void
sd_document_free(sd_document *doc)
{
	size_t i;

	for (i = 0; i < (size_t)doc->work_bufs[BUFFER_SPAN].asize; ++i)
		sd_buffer_free(doc->work_bufs[BUFFER_SPAN].item[i]);

	for (i = 0; i < (size_t)doc->work_bufs[BUFFER_BLOCK].asize; ++i)
		sd_buffer_free(doc->work_bufs[BUFFER_BLOCK].item[i]);

	sd_stack_uninit(&doc->work_bufs[BUFFER_SPAN]);
	sd_stack_uninit(&doc->work_bufs[BUFFER_BLOCK]);
